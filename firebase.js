...
// ══════════════════════════════════════════════════════════
// PERFIL DE AMIGO — solo métricas agregadas (racha, % de hoy,
// horas/día de su rutina, días perfectos). Nunca se devuelve el
// detalle de sus actividades (nombres, horarios, colores).
// ══════════════════════════════════════════════════════════
...
export async function getFriendStats(uid) {
  ...
  if (!snap.exists()) return null;
  const d = snap.data();
  const mode = d.dhv_mode || 'easy';
  const totalStars = parseInt(d.dhv_total_stars || '0');

  let acts = [];
  try { acts = JSON.parse(d.dhv_activities || '[]'); } catch { acts = []; }
  let hist = [];
  try { hist = JSON.parse(d.dhv_history || '[]'); } catch { hist = []; }

  let hoursPerDay = 0;
  acts.forEach(a => {
    const s = timeToMinutes(a.start), e = timeToMinutes(a.end);
    const dur = e <= s ? (e + 1440 - s) : (e - s);
    hoursPerDay += dur / 60;
  });

  const todayTotal = acts.length;
  const todayDone = acts.filter(a => a.done).length;
  const todayPct = todayTotal ? Math.round((todayDone / todayTotal) * 100) : null;

  let streak = (todayTotal >= MIN_ACTS_FOR_PERFECT && todayPct === 100) ? 1 : 0;
  ...
  for (let i = 1; i <= 365; i++) {
    ...
    const pct = entry.total ? Math.round(((entry.done || 0) / entry.total) * 100) : 0;
    if (pct === 100 && entry.total >= MIN_ACTS_FOR_PERFECT) streak++; else break;
  }

  const totalPerfectDays = hist.filter(h =>
    !h.skipped && h.total >= MIN_ACTS_FOR_PERFECT && Math.round(((h.done || 0) / h.total) * 100) === 100
  ).length;

  // ── Horas por semana por actividad — igual que el "Análisis de tiempo" del historial,
  // pero SOLO nombre + color + horas/semana. Nunca se calcula ni se devuelve el horario
  // (start/end) de cada actividad: no hay forma de saber a qué hora del día la hace. ──
  const totalDaysTracked = Math.max(hist.length, 1);
  const statsMap = {};
  acts.forEach(a => { statsMap[a.name] = { name: a.name, color: a.color || '#7F77DD', totalMin: 0, daysCount: 0 }; });
  hist.forEach(day => {
    if (!day.acts || day.restDay || day.skipped) return;
    day.acts.forEach(a => {
      if (!a.done) return;
      const liveAct = acts.find(x => x.name === a.name || x.id === a.id);
      const s = liveAct ? timeToMinutes(liveAct.start) : 0, e = liveAct ? timeToMinutes(liveAct.end) : 60;
      const durMin = liveAct ? (e <= s ? (e + 1440 - s) : (e - s)) : 60;
      if (!statsMap[a.name]) statsMap[a.name] = { name: a.name, color: a.color || '#7F77DD', totalMin: 0, daysCount: 0 };
      statsMap[a.name].totalMin += durMin;
      statsMap[a.name].daysCount += 1;
    });
  });
  acts.filter(a => a.done).forEach(a => {
    const s = timeToMinutes(a.start), e = timeToMinutes(a.end);
  ...
  const activityStats = Object.values(statsMap)
    .filter(s => s.totalMin > 0)
    .map(s => {
      const avgPerDay = s.totalMin / Math.max(s.daysCount, 1);
      const daysPerWeek = s.daysCount > 0 ? Math.min(s.daysCount / Math.max(totalDaysTracked / 7, 1), 7) : 1;
      return { name: s.name, color: s.color, perWeekMin: Math.round(avgPerDay * daysPerWeek) };
    })
    .sort((a, b) => b.perWeekMin - a.perWeekMin)
    .slice(0, 8);

  return {
    mode, totalStars, streak, totalPerfectDays, todayPct,
    hoursPerDay: Math.round(hoursPerDay * 10) / 10,
    activitiesToday: todayTotal,
    activityStats,
  };
}
